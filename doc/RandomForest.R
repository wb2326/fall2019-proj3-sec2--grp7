...
rf_default <- train(emotion_idx ~ .,
                    data = dat_train_selected_ratio_stand,
                    method = "rf",
                    metric = "Accuracy",
                    tuneLength = 15,
                    trControl = trControl)
...
if(run.test){
  ...
  tm_test <- system.time(pred <- predict(train_final_model_rf, dat_test_selected_ratio_stand))
}
accu <- mean(dat_test_selected_ratio_stand$emotion_idx == pred)
cat("The accuracy of model is:", accu*100, "%.\n")
confusionMatrix(pred, dat_test_selected_ratio_stand$emotion_idx)
